...
nusigmafPlut = np.array((3.48 * 1.9, 3.09 * 1.95, 2.99 * 1.83,
                         2.93 * 1.7, 2.88 * 1.67, 2.86 * 2.05))
...
PlutAbsorb = np.diag(
    sigmaGammaPlut + fissionPlut + np.sum(scatterplut, axis=0)) - scatterplut
PlutFission = np.outer(chiPlut, nusigmafPlut)


@pytest.fixture()
def plut6():
    return Fissionable("Pu239-6groups", scatterplut,
                       sigmaGammaPlut + fissionPlut,
                       nusigmafPlut, chiPlut)
...
def PlutAbsorb():
    return np.diag(
        sigmaGammaPlut + fissionPlut + np.sum(scatterplut,
                                              axis=0)) - scatterplut


@pytest.fixture()
def water():
    return Isotope("H20-1group", absorb=ar([1.983e-4]), scatter=ar([0]),
                   transport=ar([2.476]))


@pytest.fixture()
def uranium():
    return Fissionable("U235-lgroup", scatter=ar([0]), absorb=ar([8.983e-1]),
                       nusigmaf=np.array([5.925e-3]), chi=np.array([1.]),
...
def CAR_2dim():
    ...
    ref = Reflector()
    lengths = np.array(
        [0.47498, 0.34544, 1.87452, 1.87452, 1.87452, 1.87452, 1.87452,
         1.87452, 1.87452, 0.34544, 0.47625, 0.47625])
    box = Box([12, 12], boundary_conditions=[ref] * 4)
    geo = PolyBox(2, box.neighbors,
                  [[lengths[i], lengths[11 - j]] for j in range(12) for i
                   in range(12)])
    mat1 = Fissionable("1", scatter=np.array([[0, 0], [1.069e-2, 0]]),
    ...
    mat2 = Fissionable("2", scatter=np.array([[0, 0], [1.095e-2, 0]]),
    ...
    mat3 = Fissionable("3", scatter=np.array([[0, 0], [1.112e-2, 0]]),
    ...
    mat4 = Fissionable("4", scatter=np.array([[0, 0], [1.113e-2, 0]]),
    ...
    mat5 = Fissionable("5", scatter=np.array([[0, 0], [1.016e-2, 0]]),
    ...
    mat19 = Isotope("19", scatter=np.array([[0, 0], [9.095e-3, 0]]),
                    absorb=np.array([1.043e-3, 4.394e-3]),
                    transport=np.array([2.172e-1, 4.748e-1]))
    mat20 = Isotope("20", scatter=np.array([[0, 0], [3.682e-2, 0]]),
    ...
    composition = [20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20
        ...
        , 20, 19, 19, 19, 19, 19, 19, 19, 19, 19, 20, 20
        , 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20]
    names = {1: mat1, 2: mat2, 3: mat3, 4: mat4, 5: mat5, 19: mat19,
             20: mat20}
