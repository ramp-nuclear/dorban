...
from dorban.nem.cmfd_current_calculator import _compute, CMFDCurrentCalculator
from dorban.utils import op_face


def diffusion_matrix(neighbors: np.array, distances: np.array, surface_areas: np.array, E: int,
                     current_calc: CMFDCurrentCalculator) -> spar.coo_matrix:
    r"""
    ...
    C = neighbors.shape[0]
    row_indices, col_indices, values = _diffusion_matrix(neighbors, distances, surface_areas, E,
                                                         current_calc.dc, current_calc.df, current_calc.coupling, C)
...
@numba.njit()
def _diffusion_matrix(neighbors, distances, surface_areas, E: int,
                      dc, df, coupling, C) -> tuple[list[np.array], list[np.array], list[np.array]]:
    row_indices = []
    col_indices = []
    values = []
    for cell in range(C):
        ...
        for j, neighbor in enumerate(adj):
            if neighbor >= 0:
                r, c, v = _compute(cell, E, neighbor, dc[cell],
                                   dc[neighbor],
                                   distances[cell, j],
                                   distances[neighbor, j],
                                   df[cell, j], df[neighbor, op_face(j)],
                                   surface_areas[cell, j],
                                   coupling[cell, j])
            elif neighbor == -1:
                area = surface_areas[cell, j]
                r, c, v = (np.arange(cell * E, (cell + 1) * E),
