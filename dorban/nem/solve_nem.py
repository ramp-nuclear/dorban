...
from dorban.system import mesh_refinement, Core
import logging
...
class NEMSettings(Settings):
    ...
    def __init__(self, split=None,
                 ...
                 initial_k: float = 1,
                 k_tol: float = 1e-6, flux_rtol: float = 1e-5,
                 flux_atol: float = 1e-5,
                 lin_solve_rtol: float = 1e-10, lin_solve_atol: float = 1e-10,
                 max_iter=np.inf, save_file: str = None,
                 coupling=None):
        super(NEMSettings, self).__init__(False, initial_flux, initial_k, k_tol,
                                          flux_rtol, flux_atol, lin_solve_rtol,
                                          lin_solve_atol, max_iter, save_file)
...
def solve_k_nem(system: Core, settings: NEMSettings) -> Tuple[float, np.array]:
    ...
    """
    core = mesh_refinement(system,
                           settings.split) if settings.split else system
    if not settings.initial_coupling is None:
        core.current_calc.coupling = settings.initial_coupling
    solver = GeneratorNemCMFD(core,
                              settings.cmfd_iter,
                              settings.initial_k, settings.initial_flux,
                              settings.k_tol, settings.flux_rtol,
                              settings.flux_rtol,
                              settings.lin_solve_rtol,
                              settings.lin_solve_atol, settings.max_iter)
    for k, fine_flux in solver:
        logger.info(f'k number {solver.iter} is {k}')
        continue
    if settings.split is None:
        return k, fine_flux
    assemblies = system.geometry.array_refine(np.arange(system.geometry.cells),
                                              settings.split)
    volumes = system.geometry.array_refine(system.geometry.volumes,
                                           settings.split)
    normalized_fine_flux = fine_flux * np.repeat(
        core.geometry.volumes / volumes, core.E)
    assemblies_with_energy = np.vstack(
        [assemblies * core.E + e for e in range(core.E)]).flatten(
        order="F").astype(int)
...
def solve_source_nem(system: Core, settings: NEMSettings, source: np.array) -> np.array:
    ...
    solver = GeneratorNemCMFDSource(core, refined_source,
                                    settings.cmfd_iter,
                                    settings.initial_k, settings.initial_flux,
                                    settings.k_tol, settings.flux_rtol,
                                    settings.flux_rtol,
