...
@numba.njit()
def nem_rhs(E: int, flux1: np.array, flux2: np.array, transverse_leakage: np.array, absorber1: np.array,
            absorber2: np.array,
            fission1: np.array, fission2: np.array, transverse_leakage_right: np.array,
            transverse_leakage_left: np.array, transverse_length: np.array, dis1: np.array, dis2: np.array,
            transverse_leakage_righter: np.array,
            l: float, l_right: float, l_left: float, l_righter: float, source1: np.array,
            source2: np.array) -> np.array:
    r"""
    ...
    for i, length in enumerate(transverse_length):
        ...
        rhs[np.hstack((np.arange(E, 2 * E),
                       np.arange(4 * E, 5 * E)))] += _leakage_rhs(length, cell,
                                                                  left, right, l,
                                                                  l_left, l_right)
        cell = transverse_leakage_right[i]
        left = transverse_leakage[i]
        right = transverse_leakage_righter[i]
        rhs[np.hstack((np.arange(3 * E, 4 * E),
                       np.arange(5 * E, 6 * E)))] += _leakage_rhs(length, cell,
                                                                  left, right,
                                                                  l_right,
                                                                  l, l_righter)
...
@numba.njit()
def nem_boundary_rhs(E: int, flux: np.array, transverse_leakage: np.array, absorber: np.array, fission: np.array,
                     transverse_leakage_right: np.array, transverse_leakage_left: np.array, transverse_length: np.array,
                     l: float, l_right: float, l_left: float, source: np.array, boundary_rhs: np.array):
    r"""
    Function used to compute the right hand side of the equations of the nodal
    ...
    for i, length in enumerate(transverse_length):
        ...
        left = transverse_leakage_left[i]
        right = transverse_leakage_right[i]
        rhs[E:3 * E] += _leakage_rhs(length, cell, left, right, l, l_left, l_right)
    rhs[3 * E:] = boundary_rhs
    return rhs


@numba.njit()
def _leakage_rhs(length, cell_leakage, left_leakage, right_leakage, l, l_left,
                 l_right):
    c1, c2 = leakage_coefficients(cell_leakage, left_leakage, right_leakage, l,
                                  l_left, l_right)
